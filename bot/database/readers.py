...
async def get_channel_logs(
    bot: "PeterBot", guild_id: int, channel_id: int
) -> List[Mapping[str, Any]]:
    """
    Retrieves a list of user log data

    Parameters
    ----------
    bot : PeterBot
    guild_id : int
        the guild id we want to pull logs from
    channel_id : int
        the channel id we want to pull logs from

    Returns
    -------
    list
        a dictionary of objects containing message data
        [
            {
                'guild_id' : int,
                'user_id' : int,
    ...
    async with bot.db_pool.acquire() as conn:
        ...
        results = [
            ...
            for row in await query.fetch(guild_id, channel_id)
        ]
    return results
